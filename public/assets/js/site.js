...
$(document).ready(function() {
  function createJsTree(project_name) {
    ...
    $("#browser").jstree({
      ...
      types: {
        ...
        types: {
          root: {
            ...
            valid_children: [ "file-html", "folder" ],
            max_depth: 2,
            hover_node: false
          },
          "file-html": {
      ...
      plugins: [ "themes", "json_data", "contextmenu", "types", "crrm"],
      contextmenu: {
        items: function(node) { 
          var re = /^file.*/;
          return {
  ...
  function checkQuality() {
    ...
    if (JSHINT(myCodeMirror.getValue(), options)) reportSuccess(JSHINT.data()); else reportFailure(JSHINT.data());
  ...
  function commentSelection(isComment) {
    ...
    if (myCodeMirror.somethingSelected()) {
      ...
      if (isComment) comment_error.append(_(editorMessage.errorMessage, {
        message: "Please select uncommented text you want to comment."
      })); else comment_error.append(_(editorMessage.errorMessage, {
  ...
  function createFile(ele) {
    ...
    var dialogContent = '<div style="margin: 0 auto">' + "<select>" + '<option ><a href="#"></a></option>' + '<option value=".html"><a href="#">HTML</a></option>' + '<option value=".js"><a href="#">JavaScript</a></option>' + '<option value=".css"><a href="#">CSS</a></option>' + "</select>" + '<input type="text" placeholder="Enter file name" width="100%"></input>' + "</div>";
    ...
    $("#dialog").dialog({
      ...
      buttons: [ {
        ...
        click: function() { 
            ...
            $("#browser").jstree("create",  ele, "first", { 
              data: "test1"
              }, 
              function(){$(this).dialog("close");}, true);
        }
      }, {
        text: "Cancel",
        click: function() {
      ...
      buttons: [ {
        ...
        click: function() {
          if ($("#dialog input").val() === "") {
            ...
            if (!$("#dialog .alert")[0]) $("#dialog").append(error_msg);
          } else {
            $.post("/project/new", {
  ...
  $("a[data-action=editor-console-toggle]").click(function() {
    if ($(this).attr("href") === "#show") showConsole($(this)); else hideConsole($(this));
  });
  ...
  $("a[data-action=editor-console-clean]").click(function() {
    ...
    if (consoleDiv[0].innerHTML !== "") consoleDiv[0].innerHTML = "";
  });
